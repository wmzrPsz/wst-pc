        ...
        <div class="content ez-travel-car ez-conventional ez-container text-darkgray main_content">
            <div class="tourism-info">
                <ul style="width:100%;overflow: hidden;">
                    <li style="width: 570px;">
                        ...
                        <select name="" class="form-control" style="width:250px;float: left;">
                                <option value="">城市：{{startCityName}}</option>
                         </select>
                        <select name="" class="form-control" style="width:250px;float: left;">
                                <option value="">详细地址：{{startAddress}}</option>
                         </select>
                        <select name="" class="form-control" style="width:250px;float: left;">
                                <option value="">航班号{{airNo}}</option>
                         </select>
                    </li>
                ...
                <div class="tourism-info-btn">
                    ...
                    <button class="btn btn-primary"  @click="goToOrder">确定车辆</button>
...
export default {
    ...
    computed: {
        ...mapState("carTran",["orderType","airNo","startCity","startCityName","startAddress","sendAirport","sendDate","reachAirport","reachDate",
    ...
    methods: {
        ...
        commonPage2: function () {
            ...
            $('#commonPage2').jqPaginator({
                totalPages: this.carList[this.commonIndex1].commentList[this.commonIndex2].pages,
                visiblePages: 6,
                currentPage: 1,
        ...
        async selectComment(type, pageNo = 1) {
            let data = await selectComment({
                ...
                proType: 3,  //1.包车租车2.短程接送3.接送机4常规路线5.当地参团6.游轮7.景点门票
        ...
        async getRefundInfo(carid, e) {
            ...
            let data = await refundInfor({
                productType: 3,  //1.包车租车2.短程接送3.接送机4常规路线5.当地参团6.游轮7.景点门票8.当地玩家9.酒店10.保险
                productid: carid,
            })
            if (data) {
                let refundMsg = "";
        ...
        async getCarByCity(type, pageNo = 1) {
            ...
            let data = await selectShortCar({
                ...
                carType: 3,  //1.包车租车2.短程接送3.接送机4.定制
